...
#### 復習: カスタムツールのビジネス価値とMCPの本質

##### MCPの核心的な価値

```
┌─────────────┐      ┌─────────────┐      ┌─────────────┐
│  社内API     │      │  MCPサーバー │      │  AI + 自然言語 │
│  (技術的)    │  →   │  (ラッパー)  │  →   │  (誰でも使える) │
└─────────────┘      └─────────────┘      └─────────────┘
   エンジニア           エンジニアが           非エンジニアが
   しか使えない          一度作れば            自然言語で利用
```
...
| Before (API直接) | After (MCP経由) |
|------------------|-----------------|
| `curl -X GET https://api.example.com/employees?...` | 「営業部の社員一覧を見せて」 |
| `POST /expenses {"amount": 5000, ...}` | 「交通費5000円を申請して」 |
| SQLクエリを書く | 「先月の売上トップ10を教えて」 |

##### 前提条件と役割分担

| 役割 | 作業 | 頻度 |
|------|------|------|
| 前提 | 社内システムがAPIを提供している | - |
| エンジニア | MCPサーバー開発・保守 | 一度 + 必要に応じて |
| 非エンジニア | 自然言語で利用 | 日常的 |
...
| シーン | カスタムツールの例 | 効果 |
|--------|-------------------|------|
| 社内システム連携 | 勤怠API、経費精算API | 「今月の残業時間は？」で自動取得 |
| データベース操作 | 顧客DB検索、在庫確認 | 「○○社の契約状況は？」で即座に回答 |
| 業務自動化 | Slack通知、チケット作成 | 「Jiraチケット作って」 |
| 独自計算ロジック | 見積計算、与信スコア | 会社固有のルールを適用 |

##### 複合操作の例

```
ユーザー: 「新入社員の田中さんのアカウントを全部セットアップして」

AIの動作:
  1. [人事API] 田中さんの情報取得
  2. [AD API] Windowsアカウント作成
  3. [Google API] Googleアカウント作成
  4. [Slack API] Slackアカウント招待
  5. [メールAPI] 完了通知を上長に送信
```

従来なら5つのシステムを個別操作 → 一言で完了

##### 公開MCPサーバーとカスタムMCPサーバー

| 観点 | 公開MCPサーバー | カスタムMCPサーバー |
|------|----------------|-------------------|
| 例 | Slack、GitHub、DB接続 | 社内システム連携 |
| 作り方 | 同じ（MCPプロトコル） | 同じ |
| 用途 | 汎用的・再利用可能 | 特定業務に特化 |
| 配布 | npm/pip等で公開 | 社内利用のみ |

##### MCPのメリットまとめ

| メリット | 説明 |
|----------|------|
| **学習コスト削減** | API仕様書を読む必要なし |
| **ミス防止** | パラメータ形式ミスをAIが吸収 |
| **複合操作** | 複数APIをまたぐ操作も一言で |
| **アクセス制御** | ツール単位で権限管理可能 |
